...
Core.DIR_CP           = path.join(__dirname, "/../node_modules/@webxr-input-profiles/assets/dist/profiles");
...
Core.SK_INDEX		  = path.join(__dirname, "/../index_sketchfab.html");
Core.DEMO_VIEWER      = path.join(__dirname, "/../demo.html")
Core.SK_VIEWER		  = path.join(Core.DIR_PUBLIC, "../hathor/index.html");
Core.STD_SCENEFILE    = "scene.json";
Core.STD_ADDABLEFILE  = "objects_lst/objects.json";
...
Core.setupFlares = (app) => {
    ...
    for (let f in plugins) {
        ...
        if (P.respatterns && P.respatterns.length > 2) {
            Core.mpattern += "," + P.respatterns;
        }
...
Core.init = () => {
    ...
    Core.config = Core.loadConfigFile("main.json");
    Core.users = Core.loadConfigFile("users.json");
...
Core.addToUserCollection = (entry) => {
    if (entry === undefined) return false;

    // Add new entry into users json
    Core.users = Core.loadConfigFile("users.json");
    for (let i in Core.users) {
        if (Core.users[i].username === entry.username) {
            Core.users[i].items.push(entry.obj)
        }
    }
    // Core.users.push(entry);
    ...
    console.log("Updated item list with: " + entry.obj + "\nUser id: " + entry.username);
...
Core.getCollectionFolder = (sid) => {
    return path.join(Core.DIR_COLLECTIONS, sid);
};
Core.getSceneJSONPath = (sid) => {
    return path.join(Core.getSceneFolder(sid), Core.STD_SCENEFILE);
};
Core.getAddableJSONPath = (sid) => {
    return path.join(Core.getCollectionFolder(sid), Core.STD_ADDABLEFILE);
};
Core.getPubFilePath = (sid) => {
    return path.join(Core.getSceneFolder(sid), Core.STD_PUBFILE);
};

// Check if scene exists on disk
Core.existsScene = (sid) => {
    return fs.existsSync(Core.getSceneJSONPath(sid));
};
...
Core.generateUserSID = () => {
    ...
    return yyyy + mm + dd + '-' + Math.random().toString(36).substr(2, 9);
};
...
Core.applySceneEdit = (sid, patch, mode) => {
    ...
    if (S === undefined) { // scene does not exist
        return;
    }

    //jsonpatch.applyPatch(S, patch);
    // console.log("MOOOOOOOOOOOOOOOOODE: ", mode)
    if (mode === "DEL") {
        S = Core.deleteOBJEdit(S, patch);
    } else {
        S = Core.addOBJEdit(S, patch);
    }
...
Core.wappDataEdit = (wappid, fid, patch, mode) => {
    ...
    if (mode === "DEL") D = Core.deleteOBJEdit(D, patch); else D = Core.addOBJEdit(D, patch);
...
Core.createClientUserAuthResponse = (req) => {
    ...
    U.sketchfab_token = req.user.sketchfab_token

    U.items = req.user.items
...
Core.findById = (id, cb) => {
    process.nextTick(() => {
        ...
        if (Core.users[id]) cb(null, Core.users[id]); else cb(new Error('User ' + id + ' does not exist'));
    });
};
...
Core.realizeAuth = (app) => {
    ...
    app.use(session({
        secret: 'shu', //cookie: { maxAge: 1800000 }, // 60000 = 1 min
...
Core.setupDataRoute = (app) => {
    ...
    if (Core.config && Core.config.data) {
        ...
        if (D.collections) {
            Core.DIR_COLLECTIONS = D.collections;
        }
        if (D.scenes) {
            Core.DIR_SCENES = D.scenes;
        }
    }
