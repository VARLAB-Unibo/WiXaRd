...
Photon.onNewConnection = (socket) => {
    ...
    socket.on('SSTATE', () => {
        ...
        if (session !== undefined) {
            ...
            sinfo.snodes = session.snodes
            sinfo.annotationPoints = session.annotationPoints
            sinfo.annotationsIndex = session.annotationsIndex
        } else {
            sinfo.numUsers = 0;
        }
    ...
    // Update Scene State
    socket.on("UPDATE_SCENE_STATE", (node_data) => {
        const nid = node_data.nid
        const pos = node_data.pos
        const rot = node_data.rot
        const scale = node_data.scale
        const materialId = node_data.materialId
    ...
    socket.on("UPDATE_ANNOTATION_STATE", (data) => {
        const point = data.point

        const annotationsIndex = data.annotationsIndex

        if (session) {
            if (annotationsIndex > session.annotationsIndex) {
                session.annotationsIndex = annotationsIndex
                session.annotationPoints[annotationsIndex] = []
            }

            session.annotationPoints[annotationsIndex].push(point)
        }
    });
};
