...
// const { fetchProfile, MotionController } = require('@webxr-input-profiles/motion-controllers');
...
app.get(/^\/api\/collection\/(.*)$/, (req,res,next)=>{
	console.log("CI ENTRAAAAAAAAAAAAAAAAAAAAAAAAAAA")
	let sid = req.params[0];

	let ajsonpath = Core.getAddableJSONPath(sid);

	console.log("AJSONPATH: ", ajsonpath)
	console.log("EXIST PATH: ", fs.existsSync(ajsonpath))
...
app.get(/^\/api\/scene\/(.*)$/, (req,res,next)=>{
	...
	console.log("SJSONPATH: ", sjsonpath)
	...
	/*app.get("/api/collection/samples/varlab", (req,res,next)=>{
		console.log("CI ENTRAAAAAAAAAAAAAAAAAAAAAAAAAAA")
		let sid = req.params[0];
...
app.post('/api/edit/scene', (req, res) => {
	// console.log("AOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
	// Only auth users
	// TODO: check for remote users
	/*if (req.user === undefined){
		res.send(false);
		return;
	}*/
	...
	// console.log("MODEEE da API: ", mode)
...
app.post('/api/addtocollection', (req, res) => {
	...
	let b = Core.addToUserCollection(O);
	let U = Core.createClientUserAuthResponse(req);
	// console.log("U: ", U.items)
	U.items.push(O.obj);
	// console.log("U DOPO: ", U.items)
	res.send(U.items);
});
...
app.get("/api/controllerprofiles", (req, res) => {
