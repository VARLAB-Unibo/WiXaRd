...
Photon.enableChatLog = () => {
    Photon._elChat = $("<div id='idChatBox' class='atonPhotonChatBox'></div>").text("");
};
...
Photon.fireEvent = (evtname, data) => {
    if (!Photon._connected) {
        return;
    }
    let sock = Photon.socket;

    if (sock) {
        sock.emit(Photon.REPLICATED_EVT, {e: evtname, d: data});
    }
...
Photon.log = (d) => {
    ...
    if (sock) {
        sock.emit("UMSG", d); //sock.emit("LOG", d);
    }
};
...
Photon.updateSceneState = (nid, materialId = "") => {
    if (!Photon.socket) {
        return;
    }
    ...
    if (node) {
        Photon.socket.emit("UPDATE_SCENE_STATE", {
            "nid": nid,
...
Photon.updateAnnotationState = (point, annotationsIndex) => {
    if (!Photon.socket) {
        return;
    }

    Photon.socket.emit("UPDATE_ANNOTATION_STATE", {
        "point": point,
...
Photon.connect = (ssid) => {
    ...
    if (window.location.protocol === "https:") {
        opts.path = '/sphoton/socket.io';
        opts.secure = true;
        opts.rejectUnauthorized = false;
        //opts.transports = ['websocket'];
        //opts.upgrade = false
    } else {
        opts.path = '/photon/socket.io';
        //opts.transports = ['websocket'];
        //opts.upgrade = false;
    }
...
Photon.setMessage = (msg) => {
    ...
    if (Photon._elChat) {
        ...
        Photon.appendToChatBox("<div class='" + ATON.FE.getPhotonClassFromID(Photon.uid) + " atonPhotonChatUsername'>YOU</div>: <span style='color:" + Photon.ucolorhex_light[Photon.uid % 6] + "'>" + msg + "</span>");
    }
};
...
Photon.appendToChatBox = (text) => {
    ...
    Photon._elChat.append("<div class='atonPhotonChatMessage'>" + text + "</div>");
...
Photon._registerSocketHandlers = () => {
    ...
    Photon.socket.on('connect', () => {
        ...
        ATON.fireEvent("Photon_Connected");
    ...
    Photon.socket.on('disconnect', () => {
        ...
        ATON.fireEvent("Photon_Disconnected");
    });
    ...
    Photon.socket.on('ID', (data) => {
        ...
        ATON.fireEvent("Photon_IDassigned", data);
    });
    ...
    Photon.socket.on('SSTATE', (data) => {
        ...
        ATON.fireEvent("Photon_SceneState", data);
    });
    ...
    Photon.socket.on('UENTER', (data) => {
        ...
        //Photon._numUsers++;
        Photon.requestSceneState();
        ATON.fireEvent("Photon_UserEnter", uid);
    });
    ...
    Photon.socket.on('ULEAVE', (data) => {
        ...
        Photon.requestSceneState();
        ATON.fireEvent("Photon_UserLeave", uid);
    });
    ...
    Photon.socket.on('UNAME', (data) => {
        ...
        ATON.fireEvent("Photon_UName", data);
    });
    ...
    Photon.socket.on('UMSG', (data) => {
            ...
            let A = Photon.touchAvatar(uid);
            if (msg.length < 100) {
                A.setMessage(msg); // only short messages for 3D label
            }
            ...
            Photon.appendToChatBox("<div class='" + ATON.FE.getPhotonClassFromID(uid) + " atonPhotonChatUsername'>" + uname + "</div>: <span style='color:" + col2 + "'>" + msg + "</span>");

            ATON.fireEvent("Photon_UMessage", data);
        }
    )
    ;
    ...
    Photon.socket.on('UTALK', (data) => {
        ...
        ATON.fireEvent("Photon_UTalk", data);
    ...
    Photon.socket.on('UTALKSTOP', (data) => {
        ...
        ATON.fireEvent("Photon_UTalkStop", data);
    });
    ...
    Photon.socket.on('UVIDEO', (data) => {
        ...
        // MediaFlow.realizeOrUpdateVStream(data.video) // Added by me

        ATON.fireEvent("Photon_UVideo", data);
    });
    ...
    Photon.socket.on('UVIDEOSTOP', (data) => {
        ...
        ATON.fireEvent("Photon_UVideoStop", data);
    });
};
...
Photon.setFocusStreaming = (b) => {
    ...
    if (b) {
        if (!Photon._bStreamFocus) {
            ATON.fireEvent("Photon_FocusStreamingStarted");
        ...
        if (Photon._bStreamFocus) {
            ATON.fireEvent("Photon_FocusStreamingStopped");
...
Photon.touchAvatar = (uid) => {
    ...
    if (Photon.avatarList[uid] === undefined) {
        ...
        ava.loadRepresentation(ATON.PATH_RES + "models/photon/head.gltf");
