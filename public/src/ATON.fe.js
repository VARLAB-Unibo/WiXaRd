...
FE.realize = () => {
    ...
    if (dynd && dynd > 0) {
        ATON.toggleAdaptiveDensity(true);
    }
...
FE.loadSceneID = (sid, onSuccess) => {
    ...
    // console.log("ROOT VISIBLE: ", ATON._rootVisible);
};

FE.loadSceneAddables = (sid, onSuccess) => {
    if (sid === undefined) {
        return;
    }
    let sid_tmp = sid.split("/");
    const new_sid = sid_tmp[0] + "/models/" + sid_tmp[1];

    let reqstr = ATON.PATH_RESTAPI_COLLECTION + new_sid;
    // let reqstr = ATON.PATH_RESTAPI_SCENE + sid;
    //if (ATON.SceneHub._bEdit) reqstr += ",edit";

    // console.log("ADDABLES REQSTR: ", reqstr)
    // console.log("sid: ", sid)
    ATON.SceneHub.loadAddables(reqstr, sid, onSuccess);
        ...
        if (v.y) {
            if (HATHOR._actState !== HATHOR.SELECTION_EDITNODES) {
                if (v.y > 0){
                    ...
                    let camera = ATON._renderer.xr.getCamera().cameras[0]
                    let rig_position = ATON.XR.rig.position

                    camera.getWorldDirection(world_direction);
                    rig_position.add(world_direction.multiplyScalar(0.1))

                    ATON.Nav.requestPOV(new ATON.POV().setPosition(rig_position.x, rig_position.y, rig_position.z), 0.0);
                ...
                /*if (v.x > 0) {
                    ...
                    sideMoveVector.crossVectors(camera.up, world_direction);
                    sideMoveVector.normalize();

                    const movement = sideMoveVector.multiplyScalar(-0.1)

                    rig_position.add(movement)
                ...
                if (ATON.actualEditingNode !== undefined) {
                    switch (ATON._lastSelectedEditMode) {
                        case "scale":
                            if (ATON.FE._oldvY) {
                                // console.log(`INCREMENTA O DECREMENTA? : ${ATON.FE._oldvY - v.y}`)
                                if (ATON.FE._oldvY - v.y > 0) {
                                    if (ATON.actualEditingNode.isImported){
                                        let pivotNode = ATON.actualEditingNode.parent
                                        pivotNode.decrementScale();
                                    }
                                    else {
                                        ATON.actualEditingNode.decrementScale();
                                    }
                                }
                                if (ATON.FE._oldvY - v.y < 0) {
                                    if (ATON.actualEditingNode.isImported){
                                        let pivotNode = ATON.actualEditingNode.parent
                                        pivotNode.incrementScale();
                                    }
                                    else {
                                        ATON.actualEditingNode.incrementScale();
                                    }
                                }
                            }
                            ATON.FE._oldvY = v.y;
                        ...
                        case "rot":
                            if (ATON.FE._oldvY) {
                                ...
                                if (ATON.FE._oldvY - v.y > 0) {
                                    if (ATON.actualEditingNode.isImported){
                                        let pivotNode = ATON.actualEditingNode.parent
                                        pivotNode.negativeRot();
                                    }
                                    else {
                                        ATON.actualEditingNode.negativeRot();
                                    }
                                }
                                if (ATON.FE._oldvY - v.y < 0) {
                                    if (ATON.actualEditingNode.isImported){
                                        let pivotNode = ATON.actualEditingNode.parent
                                        pivotNode.positiveRot();
                                    }
                                    else {
                                        ATON.actualEditingNode.positiveRot();
                                    }
                                }
                            }
                            ATON.FE._oldvY = v.y;
...
//         FE.uiAddButton("idTopToolbar", "vr", ATON.XR.toggle);
//     }
//
//     FE.uiAddButton("idBottomToolbar", "home", () => {
...
FE.uiAddButton = (idcontainer, icon, onPress, tooltip) => {
    ...
    // let el = el = $("<div id='" + elid + "' class='atonBTN' ><img src='" + iconurl + "'></div>");
    let el = undefined
    if (icon === 'sketchfab-logo') {
        el = $("<div id='" + elid + "' class='atonBTN' data-bs-toggle='offcanvas'\n" +
            "data-bs-target='#offcanvasBottom' aria-controls='offcanvasBottom'>" +
            "<img src='" + iconurl + "'></div>");
    } else {
        el = $("<div id='" + elid + "' class='atonBTN' ><img src='" + iconurl + "'></div>");
    }
    ...
    if (onPress) {
        el.click(onPress); //$("#"+elid).click( onPress );
    }
    if (tooltip) {
        el.attr("title", tooltip); //$("#"+elid).attr("title", tooltip);
    }
...
FE.uiSwitchButton = (iconid, b) => {
    if (b) {
        $("#btn-" + iconid).addClass("switchedON");
    } else {
        $("#btn-" + iconid).removeClass("switchedON");
    }
};
...
FE.uiAddButtonBack = (idcontainer, url) => {
    FE.uiAddButton(idcontainer, "back", () => {
        if (url && url.length > 1 && url.startsWith("http:")) ATON.Utils.goToURL(url); else history.back();
    }, "Go Back");
};
...
FE.uiAddButtonFirstPerson = (idcontainer) => {
    ...
    if (ATON.Nav.isFirstPerson()) FE.uiSwitchButton("fp", true); else FE.uiSwitchButton("fp", false);
};

FE.uiAddButtonFirstPersonWalking = (idcontainer) => {
    FE.uiAddButton(idcontainer, "fpw", () => {
        if (ATON.Nav.isFirstPerson()) {
            ATON.Nav.setOrbitControl();
            //ATON.Nav.restorePreviousNavMode();
            FE.uiSwitchButton("fpw", false);
        } else {
            ATON.Nav.setFirstPersonControlWalking();
            FE.uiSwitchButton("fpw", true);
        }
    }, "First-person walking navigation mode");

    if (ATON.Nav.isFirstPerson()) FE.uiSwitchButton("fpw", true); else FE.uiSwitchButton("fpw", false);
};
...
/**
 ...
 @param {string} idcontainer - the id of html container (e.g.: "idTopToolbar")
 */
FE.setImmersiveVR = () => { // TODO creata da me, da fare bene
    if (!ATON.Utils.isConnectionSecure()) {
        return;
    }
...
FE.uiAddButtonTalk = (idcontainer) => {
    ...
    FE.uiAddButton(idcontainer, "talk", FE.talkButtonPressed, "Talk ON/OFF");
...
// Get css class from Photon ID
FE.getPhotonClassFromID = (uid) => {
    let i = (uid % 6);
    return "atonPhotonU" + i;
};

// Setup Photon events
FE._setupPhotonEvents = () => {
    if (FE._bPhotonSetup) {
        return;
    }
    ...
    ATON.on("Photon_IDassigned", (uid) => {
        ...
        $("#btn-photon").addClass(FE.getPhotonClassFromID(uid));
        ...
        FE.checkAuth((data) => {
            if (data.username !== undefined /*&& ATON.Photon._username===undefined*/) {
    ...
    ATON.on("Photon_SceneState", (sstate) => {
        console.log("Photon_SceneState")

        let numUsers = ATON.Photon.getNumUsers();
        if (numUsers > 1) {
            $("#idPhotonNumUsers").html(numUsers);
        } else {
            $("#idPhotonNumUsers").html("");
        }
    ...
    */
    ATON.on("Photon_Disconnected", () => {
        console.log("Photon_Disconnected")
        $("#btn-photon").attr("class", "atonBTN");
        // Selector color
...
FE.uiAddButtonPhoton = (idcontainer) => {
    FE.uiAddButton(idcontainer, "photon", FE.photonButtonPressed, "Photon (collaborative session)");

    $("#btn-photon").append("<span id='idPhotonNumUsers' class='atonPhotonCounter'></span>");
    ...
    if (ATON.Photon.uid !== undefined) {
        $("#btn-photon").addClass(FE.getPhotonClassFromID(ATON.Photon.uid));
    } else {
        $("#btn-photon").attr("class", "atonBTN");
    }
};

FE.photonButtonPressed = () => {
...
FE.uiAddButtonSketchfab = (idcontainer) => {
    FE.uiAddButton(idcontainer, "sketchfab-logo", () => {
        console.log("sketchfab-tab")
        // FE.popupUser();
    }, "Sketchfab Menu");
}
...
FE.uiAddButtonEditMode = (idcontainer) => {
    FE.uiAddButton(idcontainer, "edit", () => {
        console.log("ADD EDIT BUTTON", idcontainer)
        FE.checkAuth((data) => {
...
FE.switchNode = (nid, value, type) => {
    ...
    if (type === ATON.NTYPES.SEM) N = ATON.getSemanticNode(nid); else N = ATON.getSceneNode(nid);

    if (N === undefined) return;

    N.toggle(value);

    ATON.fireEvent("FE_NodeSwitch", {nid: nid, type: type, value: value});
    //console.log("XXX");
};
...
FE.uiShowObjects = () => {
    const objects_url = ".. /varlab/objects_lst/objects.json";
...
FE.playAudioFromSemanticNode = (semid) => {
    ...
    if (FE._auSemNode === undefined || FE._auSemNode === null) {
        FE._auSemNode = new THREE.Audio(ATON.AudioHub._listener);
    } else {
        FE._auSemNode.stop();
    }
...
FE.popupClose = (bNoAnim) => {
    ...
    if (bNoAnim === true) $("#idPopup").hide(); else $("#idPopup").hide();
    //$("#idPopup").empty();
...
FE.popupScreenShot = () => {
    ...
    FE.checkAuth((r) => {
        ...
        if (ATON.MediaFlow._bScreenRec) $("#btnScreenRec").addClass("atonBTN-rec"); else $("#btnScreenRec").removeClass("atonBTN-rec");
        ...
        $("#btnSetCover").click(() => {
            ...
            ATON.Utils.postJSON(ATON.PATH_RESTAPI + "cover/scene/", {
                sid: ATON.SceneHub.currID, img: cover.src
            }, (r) => {
                console.log(r);
            });
...
FE.popupPhoton = () => {
    ...
    if (numUsers > 1) {
        htmlcontent += "<div class='atonPopupTitle'>Collaborative Session (" + numUsers + " users)</div>";
    } else {
    ...
    htmlcontent += "<input id='idPhotonUsername' type='text' size='10' placeholder='username...' style='display:none'>";
    htmlcontent += "<div id='idPhotonUsernameBTN' class='atonBTN' style='width:150px; display:none'>" + ATON.Photon._username + "</div>";
    htmlcontent += "<div class='atonBTN atonBTN-text' id='idPhotonDisconnect'><img src='" + ATON.FE.PATH_RES_ICONS + "exit.png'>LEAVE</div>";

    htmlcontent += "<div id='idChatBoxPopup' style='display:block'></div>";
    htmlcontent += "<input id='idPhotonMsg' style='width:90%;margin:auto' type='text' placeholder='message...'>";
    ...
    // Tools
    ATON.checkAuth((u) => {
        // console.log("LI DATI VERI: ", u)

        if (!ATON.MediaFlow._bCamStream) {
            ATON.FE.uiAddButton("idCollabTools", "screenshare", () => {
                if (!ATON.MediaFlow._bScreenStream) {
                    $("#btn-screenshare").removeClass("atonBTN-rec");
                } else {
                    $("#btn-screenshare").addClass("atonBTN-rec");
                }
        ...
        if (!ATON.MediaFlow._bScreenStream && ATON.MediaFlow.hasVideoInput()) ATON.FE.uiAddButton("idCollabTools", "camera", () => {
            if (!ATON.MediaFlow._bCamStream) $("#btn-camera").removeClass("atonBTN-rec"); else $("#btn-camera").addClass("atonBTN-rec");
        ...
        if (ATON.MediaFlow._bCamStream) {
            $("#btn-camera").addClass("atonBTN-rec");
        } else {
            $("#btn-camera").removeClass("atonBTN-rec");
        }
    });


    if (ATON.Photon._username === undefined) {
        $('#idPhotonUsername').show();
        $('#idPhotonUsernameBTN').hide();
    } else {
        $('#idPhotonUsername').val(ATON.Photon._username);
        $('#idPhotonUsername').hide();
        $('#idPhotonUsernameBTN').show();
    }

    if (ATON.Photon.uid !== undefined) {
        $('#idPhotonUsernameBTN').addClass("atonPhotonU" + (ATON.Photon.uid % 6));
    }

    $("#idChatBoxPopup").append(ATON.Photon._elChat);

    $("#idPhotonMsg").keypress((e) => {
        let keycode = (e.keyCode ? e.keyCode : e.which);
        if (keycode === 13) {
            let str = $("#idPhotonMsg").val();
    ...
    $("#idPhotonUsername").keypress((e) => {
        let keycode = (e.keyCode ? e.keyCode : e.which);
        if (keycode === 13) {
            let str = $("#idPhotonUsername").val();
            ATON.Photon.setUsername(str);

            $('#idPhotonUsername').hide();
            $('#idPhotonUsernameBTN').html(ATON.Photon._username);
            $('#idPhotonUsernameBTN').show();
        }
    });

    $("#idPhotonUsernameBTN").click(() => {
        $('#idPhotonUsername').show();
        $('#idPhotonUsernameBTN').hide();
    });

    $("#idPhotonDisconnect").click(() => {
...
FE.checkAuth = (onReceive) => {
    ATON.Utils.checkAuth((data) => {
        ...
        if (data.username !== undefined) {
            ...
            if (ATON._userObjects === undefined) ATON._userObjects = data.items;
        } else {
            $("#btn-user").removeClass("switchedON");
        }
...
FE.popupUser = () => {
    FE.checkAuth((r) => {
        if (r.username !== undefined) { // We are already logged
            ...
            $("#idLoginBTN").click(() => {
                let jstr = JSON.stringify({
                    username: $("#idUsername").val(), password: $("#idPassword").val()
                });
                ...
                $.ajax({
                    ...
                    success: (r) => {
                        // console.log("LI DATI APPENA LOGGATO: ", r);
                        if (ATON._userObjects === undefined) ATON._userObjects = r.items;
                        if (r) {
...
FE.popupNav = () => {
    ...
    // Teleporting
    htmlcontent +=
        "<div style='display:block; width:90%; min-height:50px; vertical-align:top'>" +
        "<div style='display:inline-block; width:60px; float:left' id='idNMfp'></div>" +
        "<div style='text-align:left'>Switch between first-person and orbit navigation mode</div>" +
        "<div style='display:inline-block; width:60px; float:left' id='idNMfpw'></div>" +
        "<div style='text-align:left'>Switch between first-person walking and orbit navigation mode</div>" +
        "</div>";
    ...
    if (ATON.Utils.isConnectionSecure()) {
        if (ATON.Utils.isVRsupported()) {
            ...
            htmlcontent +=
                "<div style='display:block; width:90%; min-height:50px; vertical-align:top'>" +
                "<div style='display:inline-block; width:60px; float:left' id='idNMvr'></div>" +
                "<div style='text-align:left'>Immersive VR mode</div>" +
                "</div>";
        }
        ...
        htmlcontent +=
            "<div style='display:block; width:90%; min-height:50px; vertical-align:top'>" +
            "<div style='display:inline-block; width:60px; float:left' id='idNMar'></div>" +
            "<div style='text-align:left'>Immersive AR mode</div>" +
            "</div>";
        ...
        if (ATON.Utils.isMobile()) {
            ...
            htmlcontent +=
                "<div style='display:block; width:90%; min-height:50px; vertical-align:top'>" +
                "<div style='display:inline-block; width:60px; float:left' id='idNMdevori'></div>" +
                "<div style='text-align:left'>Enable or disable device-orientation mode</div>" +
                "</div>";
        }
    }

    if (!FE.popupShow(htmlcontent)) {
        return;
    }
