...
const COSINOIDAL_DIST = function (x) {
...
Nav.init = () => {
    ...
    // POV data - holds current viewpoint data (eye, target, etc...)
    Nav._currPOV = new ATON.POV()
        .setPosition(0, 0, 0)
        .setTarget(1, 0, 0)
        .setFOV(ATON.Nav.STD_FOV);
...
Nav.getCurrentTarget = () => {
    return Nav._currPOV.target;
};

Nav.getMotionDirection = () => {
    return Nav._motionDir;
}

Nav.getXRrigPosition = () => {
    return ATON.XR._currPos;
}
...
Nav.toggleLocomotionValidator = (b) => {
    if (b) Nav._bLocValidator = true; else {
...
Nav.getLocomotionNodeInSight = () => {
    ...
    if (Nav._dirLNode === undefined) {
        Nav._dirLNode = new THREE.Vector3();
    }
...
Nav.requestTransitionToLocomotionNode = (lnode, duration) => {
    ...
    let POV = new ATON.POV()
        ...
        .setTarget(lnode.pos.x + currDir.x, lnode.pos.y + currDir.y, lnode.pos.z + currDir.z)
...
Nav.setUserControl = (b) => {
    ...
    if (Nav._controls !== undefined) {
        Nav._controls.enabled = b;
    }

    if (Nav._cOrbit) {
        Nav._cOrbit.enabled = b;
    }
    if (Nav._cFirstPerson) {
        Nav._cFirstPerson.enabled = b;
    }
...
Nav.isOrbit = () => {
    ...
    return Nav._mode === Nav.MODE_ORBIT;
...
Nav.isDevOri = () => {
    ...
    return Nav._mode === Nav.MODE_DEVORI;
};
...
Nav.setNavMode = (navmode) => {
    ...
    if (navmode === Nav.MODE_FPW) Nav.setFirstPersonControlWalking();
...
Nav.setZoomWithWheel = (b) => {
    Nav._cOrbit.enableZoom = b;
    // Nav._cFirstPerson.enableZoom = b;
}
...
Nav.setOrbitControl = () => {
    ...
    if (ATON.AudioHub._listener && Nav._camera.children.length < 1) {
        Nav._camera.add(ATON.AudioHub._listener);
    }
...
Nav.setFirstPersonControl = () => {
    ...
    if (ATON.SUI.getSelectorRadius() > 0.1) {
        ATON.SUI.setSelectorRadius(0.1); // we (re)set selector radius to 10cm
    }
    ...
    // reparent audio listener
    // console.log("ATON NAV CAMERA: ", Nav._camera)
    if (ATON.AudioHub._listener && Nav._camera.children.length < 1) {
        Nav._camera.add(ATON.AudioHub._listener);
    }
...
Nav.setFirstPersonControlWalking = () => {
    if (ATON.XR.isPresenting()) {
        return;
    }
    ...
    Nav._controls = Nav._cFirstPerson;
    // Nav._controls = new THREE.OrbitControls(Nav._camFP, ATON._renderer.domElement);;
    // Nav._controls.target.copy(Nav._camera.position);
    Nav._camera = Nav._camFP;
...
Nav.syncCurrPOV = () => {
    ...
    if (Nav._mode === Nav.MODE_FP || Nav._mode === Nav.MODE_FPW) {
...
Nav.handlePOV = () => {
    ...
    if (ATON.XR.isPresenting()) Nav.handleXRtransition(); else Nav.handlePOVtransition();
...
Nav.handlePOVtransition = () => {
    ...
    if (Nav.POVtransitionDuration <= 0.0) Nav._tPOVprogress = 1.0; else Nav._tPOVprogress = (ATON._clock.elapsedTime - Nav._tPOVcall) / Nav.POVtransitionDuration;
...
Nav.syncCurrCamera = () => {
    ...
    if (Nav._mode === Nav.MODE_FP || Nav._mode === Nav.MODE_FPW) {
        // console.log("TARGET BEFORE: ", ctrl.target)
...
Nav.requestPOV = (pov, duration, bApplyWorldScale) => {
    ...
    if (duration !== undefined) Nav.POVtransitionDuration = duration; else Nav.POVtransitionDuration = Nav.STD_POV_TRANS_DURATION;
...
Nav.computeDefaultHome = (dv, sceneBS) => {
    if (dv === undefined) {
        dv = new THREE.Vector3(1, 0.7, 1);
    }

    if (sceneBS === undefined) {
        sceneBS = ATON.getRootScene().getBound();
    }
