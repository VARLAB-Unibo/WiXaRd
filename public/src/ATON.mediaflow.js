    ...
    navigator.mediaDevices.getUserMedia(MediaFlow._cAuStream)
        .then((stream) => {
            ...
            MediaFlow._aurec.ondataavailable = (e) => {
                ...
                if (MediaFlow._schunks.length < MediaFlow.auStreamNumSegments) {
                    return;
                }

                if (MediaFlow._aurec.state !== "inactive") {
                    MediaFlow._aurec.stop();
                }
            };
...
MediaFlow.startScreenRecording = () => {
    if (MediaFlow._bScreenRec) {
        return;
    }
    if (MediaFlow._bScreenStream) {
        return;
    }
    ...
    navigator.mediaDevices.getDisplayMedia(MediaFlow._cScreenStream)
        .then((stream) => {
            ...
            MediaFlow._vrec.ondataavailable = event => {
                ...
                if (MediaFlow._scchunks.length < MediaFlow.vidStreamNumSegments) {
                    return;
                }

                if (MediaFlow._vrec.state !== "inactive") {
                    MediaFlow._vrec.stop();
                }
            }
...
MediaFlow.realizeOrUpdateVStream = (stream, onClick) => {
    ...
    // let vs = MediaFlow.getOrCreateVideoStream(uid, stream, true);

    vs.el.playsinline = true;
    vs.el.style.display = "inline-block";

    vs.el.classList.add("atonPhotonVidStream");
    vs.el.classList.add("atonPhotonU" + (uid % 6));

    if (onClick) {
        vs.el.onclick = onClick;
    }
...
MediaFlow.getOrCreateVideoStream = (id, sourceurl, bUser) => {
    if (!MediaFlow._vStreams[id]) {
        ...
        if (!bUser) {
            MediaFlow._vStreams[id].el.loop = true;
        }
