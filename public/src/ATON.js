...
import SUI from "./ATON.sui/ATON.sui.js";
...
// import Work from "./ATON.worker.js";
//import NX from "./_prv/ATON.nx.js";
//import Period from "./ATON.period.js";
import KEYBOARD from "./ATON.sui/ATON.sui.keyboard.js"
...
ATON.oldEvX = 0;
ATON.oldEvY = 0;
ATON._draggableUIObjects = [];
ATON._userObjects = undefined;


// ATON.KEYBOARD = KEYBOARD;
...
ATON.PATH_RESTAPI_COLLECTION = `${ATON.PATH_RESTAPI}collection/`;
...
ATON.PATH_PROFILESLIST = `${ATON.BASE_URL}/dist/@webxr-input-profiles/assets/dist/profiles`;
...
ATON.SCALE_DEFAULTSCENE = 1.2;
ATON.SCALE_SCENE = 3.5;
ATON.SCALE_BIG = 5;
...
ATON._dragControls = undefined;
ATON.T1Click = false;
ATON.T2Click = false;
ATON.T3Click = false;
ATON.T4Click = false;
ATON._lastTimeStamp = 0.0;

// ATON.moveBackFlag = false;
...
let canvas;
const server_uri = 'https://localhost:8084/';
let requestId;
let oldMainClickFlag = false;
...
ATON._onUserInteraction = () => {
    if (ATON._elPanoVideo && !ATON._vpanoPlaying) {
        ATON._elPanoVideo.play();
    }
    if (ATON.XPFNetwork._elVid && !ATON.XPFNetwork._vidPlaying) {
        ATON.XPFNetwork._elVid.play();
    }

    if (ATON.AudioHub._listener && ATON.AudioHub._listener.context) {
        if (ATON.AudioHub._listener.context.state === 'suspended') {
            ATON.AudioHub._listener.context.resume();
        }
    }
...
ATON.checkAuth = (onLogged, onNotLogged) => {
    $.ajax({
        type: 'GET', url: ATON.PATH_RESTAPI + "user", xhrFields: {withCredentials: true}, dataType: 'json',

        success: (data) => {
            if (data !== undefined && data.username) onLogged(data); else if (onNotLogged) onNotLogged();
        },
...
*/
    ...
    //     ATON.fireEvent("MouseClick")
    //
    //     console.log(ATON._queryDataScene)
    //
    //     if (ATON._queryDataScene === undefined) {
    //         const H = ATON.getSceneNodeByComparingPositions(ATON._queryDataScene);
    //         // if (H && H.onLeave) {
    //         //     H.onLeave();
    //         // }
    //
    ...
    //
    //     const H = ATON.getSceneNodeByComparingPositions(ATON._queryDataScene);
    //     // let H = ATON.getSceneNode(ATON._hoveredScene);
    //     // // console.log("hoveredScene", ATON._hoveredScene)
    //     // // console.log("sceneNode", H)
    //     // if (H && H.onSelect) {
    //     //     // H.nowDragging = true
    //     //     H.onSelect()
    //     // }
    ...
    el.addEventListener('mousedown_remote', (e) => {
        // console.log("CLICKED: ", e.button)
        switch (e.button) {
            case 1: // left-click
                ATON._mainClick = true
                ATON.fireEvent("MouseLeftButton");
                ...
                ATON._handleOnSelect()
                break;
            case 2: // middle-click
                ATON.fireEvent("MouseMidButton");
                break;
            case 3: // right-click
                ATON.fireEvent("MouseRightButton");
                break;
        }
    ...
    el.addEventListener('mousedown', (e) => {
        ...
        switch (e.button) {
            case 1: // left-click
                ATON._mainClick = true
                ATON.fireEvent("MouseLeftButton", e);
    ...
    */
    ...
    // Funziona anche da left-click del mouse
    Hammer(el).on("tap", (e) => {
        //ATON._evPointer = e.srcEvent;
        // ATON._bPointerDown = false;
        if (!ATON.XR.isPresenting()) ATON._mainClick = true;
    ...
    window.addEventListener("keydown", (e) => {
        ...
        switch (e.key) {
            case "Shift":
                ATON._kModShift = true;
                break;
            case "Control":
                ATON._kModCtrl = true;
                break
            case "Backspace":
                ATON._kModBackspace = true
                break
        }

        if (!ATON._bListenKeyboardEvents) {
            return;
        }
    ...
    // Defaults
    ATON.on("KeyPress", (k) => {
        console.log("PUlSANTE PREMUTO: ", k)
        switch (k) {
            case '+':
                ATON.Nav.setFOV(ATON.Nav.getFOV() + 1.0);
                break
            case "-":
                ATON.Nav.setFOV(ATON.Nav.getFOV() - 1.0);
                break
            ...
            case 's':
                ...
                if (ATON.Nav._mode === ATON.Nav.MODE_FPW) {
                    ...
                    ATON.Nav._controls.target.add(target.multiplyScalar(-0.1));
                    const target_diff = old_target.sub(ATON.Nav._controls.target)
                    ATON.Nav._camera.position.sub(target_diff);
                }
                break;
            case 'a':
                // LEFT AIR WALKING
                if (ATON.Nav._mode === ATON.Nav.MODE_FPW) {
                    ...
                    ATON.Nav._camera.getWorldDirection(target);
                    sideMoveVector.crossVectors(ATON.Nav._camera.up, target);
                    sideMoveVector.normalize();
                    const movement = sideMoveVector.multiplyScalar(0.1)
                    ATON.Nav._controls.target.add(movement);
                    ATON.Nav._camera.position.add(movement);
                    // console.log("SINISTRA TARGET: ", ATON.Nav._controls.target);
                    // console.log("SINISTRA POSITION: ", ATON.Nav._camera.position);
                }
                break;
            ...
            case 'd':
                ...
                if (ATON.Nav._mode === ATON.Nav.MODE_FPW) {
                    ...
                    ATON.Nav._controls.target.add(movement);
                    ATON.Nav._camera.position.add(movement);
                    // console.log("DESTRA TARGET: ", ATON.Nav._controls.target);
                    // console.log("DESTRA POSITION: ", ATON.Nav._camera.position);
                }
                break
            case "r":
                if (ATON.actualEditingNode) {
                    ATON.actualEditingNode.setNowRotating(true)
                }
                break
            case "x":
            case "y":
            case "z":
                ATON._lastSelectedAxis = k
...
ATON._handleOnSelect = () => {
    // Scene selection
    if (ATON._hoveredSceneNode !== undefined) {
        let H = ATON.getSceneNode(ATON._hoveredSceneNode);
        if (H && H.onSelect) {
            H.onSelect();
            return true
        }
...
// E.g. double-tap, VR controllers trigger, etc.
ATON._SUIactivation = () => {
    ...
    // }
    //
    // if (U.onSelect === undefined) {
    //     return false;
    // }
    //
    // U.onSelect();
    // return true;
    return ATON._handleOnSelect()
};
...
ATON._stdActivation = () => {
    ...
    if (ATON.XR._bPresenting) {
        if (HATHOR._actState !== HATHOR.SELECTION_EDITNODES) {
            if (Nav.requestTransitionToLocomotionNodeInSightIfAny(ATON.XR.STD_TELEP_DURATION)) {
                return;
            }

            if (ATON.XR._sessionType === "immersive-vr") {
                // console.log("IMMERSIVE SESSION2")
                ATON.XR.teleportOnQueriedPoint();
            }
        }
        else {
            if (ATON.actualEditingNode) {
                if (ATON._hoveredSceneNode !== ATON.actualEditingNode.nid ){
                    if (Nav.requestTransitionToLocomotionNodeInSightIfAny(ATON.XR.STD_TELEP_DURATION)) {
                        return;
                    }
    ...
    if (bFPtrans) {
        if (Nav.requestTransitionToLocomotionNodeInSightIfAny(0.5)) {
            return;
        }
        ...
        if (ATON.Nav.currentQueryValidForLocomotion()) {
            ...
            let ftgt = new THREE.Vector3(feye.x + currDir.x, feye.y + currDir.y, feye.z + currDir.z,);
...
async function captureFrame() {
    ...
    // Replace the following line with your canvas rendering code
    // ctx.fillRect(0, 0, canvas.width, canvas.height);
    const strMime = "image/jpeg";

    const imgData = canvas.toDataURL(strMime, 0.5);

    // ATON.Worker.postMessage({img: imgData, fps:ATON._fps})

    // ATON.Worker.onmessage = function (e) {
    //     console.log('Message from the Web Worker:', e.data);
    // };
    // console.log('img size: ', calculateSize(imgData), ' KB');
    sendBufferToServer(imgData, parseInt(ATON._fps));
}
...
async function sendBufferToServer(captured_image, buffer_size/*, hier*/) { // Added \uuid as a parameter
    const data = {
    ...
    try {
        fetch(server_uri + 'imgs_manager', options);
    } catch (error) {
...
ATON.realize = () => {
    ...
    ATON._dRenderBudgetMaxFPS = 60.0;
    ...
    */
    ...
    // console.log('bNoRender: ' + bNoRender);
    ...
    canvas = ATON._renderer.domElement;
    canvas.id = "idView3D";
    ...
    */
    ...
    ATON._hoveredSceneNode = undefined;
    ...
    */
    ...
    // Ray casters
    ATON._rcSUI = new THREE.Raycaster(); // TEST
...
ATON.loadPDFManagerSetup = () => {
    ATON._loadManagerPDF = new THREE.LoadingManager();
    ATON._loadManagerPDF.onStart = (url, itemsLoaded, itemsTotal) => {
        console.log('Started loading file: ' + url + '.\nLoaded ' + itemsLoaded + ' of ' + itemsTotal + ' files.');
        ATON.fireEvent("NodeRequestFired", url);
    };

    ATON._loadManagerPDF.onLoad = () => {
        console.log('Loading complete!');
        ATON.fireEvent("PDFLoaded");
    };

    ATON._loadManagerPDF.onProgress = (url, itemsLoaded, itemsTotal) => {
        console.log( 'Loading file: ' + url + '.\nLoaded ' + itemsLoaded + ' of ' + itemsTotal + ' files.' );
    };

    ATON._loadManagerPDF.onError = (url) => {
        console.log('There was an error loading ' + url);
    };
};
...
ATON.setDefaultPixelDensity = (d) => {
    ...
    if (ATON.device.isMobile) ATON._renderer.xr.setFramebufferScaleFactor(ATON._stdpxd * ATON.XR.MOBILE_DENSITY_F); else ATON._renderer.xr.setFramebufferScaleFactor(ATON._stdpxd);
...
ATON.getorCreateUINode = (id, type = "button") => {
    let uiNode = ATON.getUINode(id)

    if (!uiNode) {
        switch (type) {
            case "button":
                uiNode = new ATON.SUI.Button(id)
                break
            case "label":
                uiNode = new ATON.SUI.Label(id)
                break
        }
...
ATON._pdfComplete = (url) => {
    ATON.fireEvent("PDFLoaded", url);
};
...
ATON.recomputeSceneBounds = (ubs) => {
    ...
    */
    ...
    // Auto-LP
    if (ATON._bAutoLP) {
        if (ATON._lps[0] === undefined) ATON.addLightProbe(new ATON.LightProbe().setPosition(ATON.bounds.center).setNear(ATON.bounds.radius)); else {
...
ATON.playOrPauseMainPanorama = () => {
    ...
    if (ATON._vpanoPlaying) ATON._elPanoVideo.pause(); else ATON._elPanoVideo.play();
}
...
ATON.toggleMainLight = (b) => {
    ...
    if (b) {
        if (numLPs > 0) ATON.setNeutralAmbientLight(0.0); else ATON.setNeutralAmbientLight(ATON.AMB_L);
...
ATON.toggleShadows = (b) => {
    ...
    if (b) {
        ...
        if (ATON.device.isMobile) ATON._renderer.shadowMap.type = THREE.PCFShadowMap; else ATON._renderer.shadowMap.type = THREE.PCFSoftShadowMap; // THREE.VSMShadowMap;
...
ATON.updateDirShadows = (/*p*/) => {
    ...
    ATON._dMainL.position.set(ATON._dMainLpos.x - ATON._dMainLdir.x, ATON._dMainLpos.y - ATON._dMainLdir.y, ATON._dMainLpos.z - ATON._dMainLdir.z);
...
ATON.setGlobalAudio = (audioURL, bLoop) => {
    ...
    if (ATON._auMain === undefined || ATON._auMain === null) ATON._auMain = new THREE.Audio(ATON.AudioHub._listener); else if (ATON._auMain.isPlaying) ATON._auMain.stop();
...
ATON._onFrame = () => {
    ...
    if (ATON.XR._bPresenting) ATON.XR.update();
        ...
        // console.log("ATON NAV: ", ATON.Nav)
        ATON.Nav._controls.update(ATON._dt);
    }

    // Spatial queries
    /*if (!ATON.device.isMobile || ATON._bCenteredQuery || ATON.XR._bPresenting)*/

    ATON.checkEmptyStacks();
    ...
    let draggableItems = ATON._draggableUIObjects.length;
    if (ATON._hoveredUI){
        const uiNode = ATON.getUINode(ATON._hoveredUI);
        if (!ATON._draggableUIObjects.includes(uiNode) && uiNode.parent.nid === ".")
            ATON._draggableUIObjects.push(uiNode);
    }
    else {
        ATON._dragControls.dispose();
        ATON._draggableUIObjects = [];
    }

    if (ATON._draggableUIObjects.length > draggableItems) {
        ATON._dragControls = new THREE.DragControls(ATON._draggableUIObjects, ATON.Nav._camera, ATON._renderer.domElement);
        ATON._dragControls.recursive = false;
        ATON._dragControls.transformGroup = true;
        ATON._dragControls.addEventListener('drag', (event) => {
            // const uiNode = ATON.getUINode(ATON._hoveredUI);
            // Make the dragged object face the camera's direction
            const draggedObject = event.object;
            draggedObject.lookAt(ATON.Nav._camera.position);
        });
    }
    if (ATON._isXRDragging) {
        const uiNode = ATON.getUINode(ATON._hoveredUI);
    ...
    // console.log('x: ' , ATON.evX , ' --- y: ' , ATON.evY);
    // ATON._screenPointerCoords.x = (ATON.evX / window.innerWidth) * 2 - 1;
    // ATON._screenPointerCoords.y = -(ATON.evY / window.innerHeight) * 2 + 1;
    ...
    console.log('Main root name: ' + ATON._mainRoot.name)
    let childrens = ATON._mainRoot.children;
    for (let node in childrens){
        console.log('child: ' + node)
    }
    console.log('Main root childrens: ' + childrens)
    console.log(SceneHub.getJSONchildren(ATON._mainRoot.name))*/
...
ATON.checkEmptyStacks = () => {
    if (ATON.undoStack.length === 0) $('#btn-undo').hide();
    else $('#btn-undo').show();

    if (ATON.redoStack.length === 0) $('#btn-redo').hide();
    else $('#btn-redo').show();
}

// Render frame
ATON._render = () => {
    if (!ATON.FX.composer || ATON.XR._bPresenting) ATON._renderer.render(ATON._mainRoot, ATON.Nav._camera); else {
...
ATON._updateScreenMove = (e) => {
    ...
    */
    ...
    if (ATON.oldEvX !== ATON.evX || ATON.oldEvY !== ATON.evY) {
        ATON._screenPointerCoords.x = (ATON.evX / window.innerWidth) * 2 - 1;
        ATON._screenPointerCoords.y = -(ATON.evY / window.innerHeight) * 2 + 1;
    } else {
    ...
    ATON.oldEvX = ATON.evX;
    ATON.oldEvY = ATON.evY;
...
ATON._handleQueries = () => {
    ...
    // do not query during POV transitions
    ...
    // Timed gaze input
    if (ATON._tgiDur === undefined) {
        return;
    }
    if (ATON._tHover === undefined) {
        return;
    }
...
//     } else {
//         ATON._rcSUI.setFromCamera(ATON._screenPointerCoords, ATON.Nav._camera);
//     }
//
//     ATON._hitsSUI = [];
//     ATON._rcSUI.intersectObjects(ATON._mainRoot.children, true, ATON._hitsSUI);
//
//     // Process hits
//     const hitsnum = ATON._hitsSUI.length;
//
//
//     const h = ATON._hitsSUI[0];
//
//     if (ATON.actualEditingNodeTest !== undefined) {
//         if (ATON.actualEditingNodeTest.nid === "sui_povnext") {
//             ATON.actualEditingNodeTest.nowDragging = true
//             ATON.actualEditingNodeTest.dragEvent(h.point)
//         }
//     }
//
//     // ATON._queryDataSUI = {};
//     // ATON._queryDataSUI.p = h.point;
//     // ATON._queryDataSUI.d = h.distance;
//     // ATON._queryDataSUI.o = h.object;
//     // ATON._queryDataSUI.list = []; // holds ui-nodes parental list
//     //
//     // // traverse parents
//     // const L = ATON._queryDataSUI.list;
//     // let sp = h.object.parent;
//     // while (sp) {
//     //     if (sp.nid && sp.nid !== ATON.ROOT_NID) {
//     //         L.push(sp.nid);
//     //     }
//     //     sp = sp.parent;
//     // }
...
//     // const hui = L[0];
//     // if (hui) {
//     //     if (ATON._hoveredSUI !== hui) {
//     //         if (ATON._hoveredSUI) {
//     //             // ATON.fireEvent("UINodeLeave", ATON._hoveredUI);
//     //             const S = ATON.getUINode(ATON._hoveredSUI);
//     //             // if (S && S.onLeave) {
//     //             //     S.onLeave();
//     //             // }
//     //
...
//     //
//     //         // if (S.nowEditing) {
//     //         //     ATON.actualEditingNodeTest = S;
//     //         // }
//     //     }
//     // }
// }

ATON.getIntersectObjects = () => {
    let found = [];
    //ATON._rcScene.intersectObjects( ATON._rootVisible.children, true, ATON._hitsScene );
    // console.log("mainroot", ATON._mainRoot)
    // console.log("children", ATON._mainRoot.children)
    ATON._rcScene.intersectObjects(ATON._mainRoot.children, true, found);
...
ATON._handleQueryScene = () => {
    ...
    // console.log("SCENA TEST", ATON._hitsScene)
    ...
    // if (SemFactory.nowAnnotating && ATON._kModCtrl) {
    //     if (SemFactory.nowWritingAnnotation) {
    //         ATON.SemFactory.writeAnnotation(ATON._queryDataScene.p)
    //     } else if(SemFactory.nowErasingAnnotation){
    //         ATON.SemFactory.removeAnnotation(ATON._queryDataScene.p)
    //     }
    // }

    ATON.fireEvent("HitsScene", ATON._queryDataScene)

    // Normals
    if (!ATON._bQueryNormals) {
        return;
    }
    if (!h.face) {
        return;
    ...
    // traverse parents
    ATON._queryDataScene.list = []; // holds scene-nodes parental list
    const L = ATON._queryDataScene.list;
    let sp = h.object.parent;
    while (sp) {
        if (sp.nid && sp.nid !== ATON.ROOT_NID) {
            L.push(sp.nid);
    ...
    if (hsn) {
        // console.log(`HSN: ${hsn}`)
        if (ATON._hoveredSceneNode !== hsn) {
            if (ATON._hoveredSceneNode) {
                ATON.fireEvent("SceneNodeLeave", ATON._hoveredSceneNode);
                let S = ATON.getSceneNode(ATON._hoveredSceneNode);
                if (S && S.onLeave) {
                    S.onLeave();
                }
            ...
            ATON._hoveredSceneNode = hsn;
            ATON.fireEvent("SceneNodeHover", hsn);
            let S = ATON.getSceneNode(hsn);
            // console.log("NIIIIIID SCENE", S.nid)
            if (S) {
                if (S.onHover) {
                    S.onHover();
                }
                ...
                //     S.onSelect;
                // }
                // console.log(`S.nowEditing: ${S.nowEditing}`)
                if (ATON._mainClick && S.editable && S.onHover) {
                    S.nowEditing = true;
                    ATON.actualEditingNode = S;
                }
            }
        } else {
            if (ATON._hoveredSceneNode === hsn){
                let node = ATON.getSceneNode(hsn);
                // console.log("hovered scene node: ", ATON._hoveredSceneNode);
                // console.log("node editing: ", sk_spawn_node.nowEditing);
                if (node !== undefined) {
                    if (node.isImporter){
                        if (ATON._mainClick && ATON._mainClick !== oldMainClickFlag) {
                            switch (HATHOR._actState) {
                                case HATHOR.SELECTION_STD:
                                    let obj_count = hsn.split('objcount')[1];
                                    HATHOR.setSpawnSketchfabImporter(undefined, node, obj_count);
                                    break;
                                case HATHOR.SELECTION_EDITNODES:
                                    if (node.editable) {
                                        // console.log(`NODE: ${node.nid}`)
                                        node.nowEditing = true;
                                        ATON.actualEditingNode = node;
                                    }
                            }
                            // if (sk_spawn_node.children[1]) console.log("sk spawn children: ", sk_spawn_node.children[1].nid)
                        }
                        oldMainClickFlag = ATON._mainClick
                    }
                }
            }
        }
    ...
    // Send to actual dragging node
    if (ATON.actualEditingNode) {
        if (ATON.XR.isPresenting()) {
            if (ATON.actualEditingNode.nid === ATON._hoveredSceneNode && !ATON._hoveredUI) {
                if (ATON._isXRDragging && ATON._lastSelectedEditMode === "drag") {
                    const P = ATON._queryDataScene.p
                    const old_pos = (ATON.actualEditingNode.isImported) ? ATON.actualEditingNode.parent.position.clone() : ATON.actualEditingNode.position.clone();
                    const stackInfo = {
                        action: "drag",
                        node: ATON.actualEditingNode,
                        position: old_pos
                    }
                    // console.log("POSITION EVENT OLD POS: ", ATON.actualEditingNode.parent.position)
                    ATON.undoStack.push(stackInfo);
                    ATON.actualEditingNode.positionEvent(P)
                    // console.log("POSITION EVENT NEW POS: ", ATON.actualEditingNode.parent.position)
                }
            }
        }
        else {
            if (!ATON._hoveredUI) {
                if (ATON._mainClick && ATON._lastSelectedEditMode === "drag") {
                    const old_pos = (ATON.actualEditingNode.isImported) ? ATON.actualEditingNode.parent.position.clone() : ATON.actualEditingNode.position.clone();
                    const stackInfo = {
                        action: "drag",
                        node: ATON.actualEditingNode,
                    ...
                    // console.log("POSITION EVENT OLD POS: ", old_pos)
                    ATON.undoStack.push(stackInfo);
                    ATON.actualEditingNode.positionEvent(h.point)
                    // console.log("DRAG STACK INFO: ", stackInfo)
                    // console.log("POSITION EVENT NEW POS: ", (ATON.actualEditingNode.isImported) ? ATON.actualEditingNode.parent.position : ATON.actualEditingNode.position)
                }
            }
        }
    }
    ...
    if (ATON._hoveredUI) {
        /*for (let idx in ATON._rootUI.children) {
            if (ATON._rootUI.children[idx].name !== "avatars" && ATON._rootUI.children[idx].name !== "") {
                if (!ATON._draggableUIObjects.includes(ATON._rootUI.children[idx]))
                    ATON._draggableUIObjects.push(ATON._rootUI.children[idx])
            }
        }*/


        // const uiNode = ATON.getUINode(ATON._hoveredUI);
        // uiNode.positionEvent()
    }

    // Drag and drop for UI
    /*if (ATON._hoveredUI && ATON.XR.isPresenting() && ATON._isXRDragging) {
        const uiNodeToDrag = ATON.getUINode(ATON._hoveredUI);
        const P = ATON._queryDataScene.p;
        uiNodeToDrag.positionEvent(P);
    }*/
};

ATON.selectHoveredNode = () => {
    const h = ATON._hitsScene[0];
    // Normals
    if (!ATON._bQueryNormals) {
        return;
    ...
    const hsn = L[0];
    const node = ATON.getSceneNode(hsn);
    console.log("HOVERED NODE BEFORE SetNowEditing (FUNCTION selectHoveredNode): ", node)
    if (node.editable){
...
ATON._handleQueryUI = () => {
    ...
    // if (SemFactory.nowAnnotating && ATON._kModCtrl) {
    //     if (ATON._kModShift) {
    //         ATON.SemFactory.removeAnnotation(ATON._queryDataUI.p)
    //     } else {
    //         ATON.SemFactory.writeAnnotation(ATON._queryDataUI.p)
    //     }
    // }

    ATON.fireEvent("HitsUI", ATON._queryDataUI)
...
// TEST
//     if (ATON._kModCtrl) {
//         // console.log("nodo", S.getPosition())
//         // console.log("h", h.point)
//         // console.log("caster", ATON._rcUI.ray)
//         h.point.z = undefined
//         h.point.x = undefined
//
//         // if (ATON._kModShift) {
...
//         }
//     }
    // console.log(ATON._rcUI.ray.origin)
    // console.log(ATON._rcUI.ray.direction)
    // console.log("----------")
...
// ATON._addBoundingBox = (helper) => {
//     ATON._rootVisibleGlobal.add(helper);
// }
