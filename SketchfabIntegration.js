	    ...
	    gltfLoader.load(fileUrl, (gltf) => {
			...
			gltf.scene.traverse(function(node) {
				...
				if (node.children.length > 0){
					child_names = node.children.map(child => child.name);
				}

				if (node.parent) anchestor_node = node.parent.name;

				scene_hierarchy.push({
					'name' : node.name,
					'parent' : anchestor_node,
					'childs' : child_names
				});
			});

			console.log(scene_hierarchy);
